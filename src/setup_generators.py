...
def setup_generator(data_dir, segmentation=False):
    ...
    # Construct 'filename' column with correct paths
    # Adjust 'data_dir' and path construction as needed
    labels['filename'] = labels['variety_image_path'].apply(lambda x: os.path.abspath(x.strip()))
    
    # Debugging: Print sample filenames
    print("\nSample 'filename' entries:")
    print(labels['filename'].head())
    
    # Use 'class' column as labels
    labels['label'] = labels['class'].astype(str).str.strip()
    
    # Verify that the image files exist
    labels['file_exists'] = labels['filename'].apply(os.path.exists)
    print("\nFile existence check:")
    print(labels[['filename', 'file_exists']].head())
    
    missing_files = labels[~labels['file_exists']]
...
if __name__ == '__main__':
    data_dir = '.'  # Adjust this path if necessary
    try:
